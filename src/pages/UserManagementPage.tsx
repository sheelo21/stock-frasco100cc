...
export default function UserManagementPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // データをマージ
      const mergedUsers: UserWithRole[] = authUsers.users.map(authUser => {
        ...
        return {
          ...
          role: (role?.role as AppRole) || "client",
          discount_rate: profile?.discount_rate || null,
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      // 権限更新
      await handleRoleChange(editingUser.user_id, "", editFormData.role);
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <div className="flex gap-2">
            <Select value={sortBy} onValueChange={(value: any) => setSortBy(value)}>
              ...
              <SelectContent>
                <SelectItem value="email">メールで並び替え</SelectItem>
                <SelectItem value="role">権限で並び替え</SelectItem>
                <SelectItem value="created_at">作成日で並び替え</SelectItem>
              </SelectContent>
            </Select>
            <Button
              variant="outline"
              ...
              {editFormData.role === "client" && (
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditFormData(prev => ({ ...prev, discount_rate: e.target.value }))}
                    placeholder="10"
                    min="0"
                    max="100"
                  />
